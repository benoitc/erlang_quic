...
%%%   * retransmits under 1% of packets, since loopback with a socket
%%%     buffer of the size the harness asks for loses nothing;
...
assert_counters(Direction, Opts, Stats, Bytes) ->
    ...
    %% The kernel clamps the harness's 7 MB buffer to rmem_max; at the
    %% Linux default loopback overflows and every branch retransmits
    %% 3 to 20%, so CI raises the limit. One percent is margin.
    ?assert(
