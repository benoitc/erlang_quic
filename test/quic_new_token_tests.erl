...
client_caches_new_token_keyed_by_remote_addr_test() ->
    ...
    Addr = {{127, 0, 0, 1}, 4433},
    S0 = quic_connection_test_support:state_for_client(Addr),
...
server_accepts_valid_new_token_test() ->
    ...
    S = quic_connection_test_support:state_for_server(Addr, secret(), ODCID),
...
server_rejects_token_from_wrong_address_test() ->
    ...
    S = quic_connection_test_support:state_for_server(AttackerAddr, secret(), <<>>),
    ?assertMatch(
...
server_accepts_retry_token_regardless_of_odcid_test() ->
    ...
    S = quic_connection_test_support:state_for_server(Addr, secret(), <<9, 9, 9>>),
    ?assertEqual(validated, quic_connection:maybe_validate_initial_token(Token, S)).

server_without_secret_skips_validation_test() ->
    Addr = {{127, 0, 0, 1}, 4433},
    S = quic_connection_test_support:state_for_server(Addr, undefined, <<>>),
    ?assertEqual(
...
empty_token_returns_no_token_test() ->
    Addr = {{127, 0, 0, 1}, 4433},
    S = quic_connection_test_support:state_for_server(Addr, secret(), <<>>),
