...
client_mirrors_received_spin_test() ->
    S0 = quic_connection_test_support:spin_state_for(client, true),
    %% Received packet with spin = 1 (bit 5 set) and PN 1 (> -1).
    S1 = quic_connection_test_support:update_spin_from_recv(2#00100000 bor 16#40, 1, S0),
    ?assertMatch(
        #{outgoing := 1, recv := 1, largest_pn := 1, enabled := true},
        quic_connection_test_support:spin_state(S1)
    ).

%% A server inverts the received spin bit.
server_inverts_received_spin_test() ->
    S0 = quic_connection_test_support:spin_state_for(server, true),
    S1 = quic_connection_test_support:update_spin_from_recv(2#00100000 bor 16#40, 1, S0),
    ?assertMatch(
        #{outgoing := 0, recv := 1},
        quic_connection_test_support:spin_state(S1)
...
reordered_packet_does_not_update_test() ->
    S0 = quic_connection_test_support:spin_state_for(client, true),
    S1 = quic_connection_test_support:update_spin_from_recv(2#00100000 bor 16#40, 5, S0),
    %% A later-arriving, earlier-PN packet with spin=0 must NOT flip us.
    S2 = quic_connection_test_support:update_spin_from_recv(16#40, 3, S1),
    ?assertEqual(
        quic_connection_test_support:spin_state(S1),
        quic_connection_test_support:spin_state(S2)
    ).

%% When disabled, outgoing stays at 0 regardless of what we receive.
disabled_keeps_outgoing_at_zero_test() ->
    S0 = quic_connection_test_support:spin_state_for(client, false),
    S1 = quic_connection_test_support:update_spin_from_recv(2#00100000 bor 16#40, 1, S0),
    ?assertMatch(
...
first_byte_respects_disable_test() ->
    S = quic_connection_test_support:spin_state_for(client, false),
